        ...
        if not step1_failed and actual_tool_result is not None:
            ...
            if answer_match:
                ...
                if actual_tool_result is not None and final_answer == actual_tool_result:
                    ...
                    print(f"Rollout {rollout_idx+1}/{num_rollouts} | Step 2 | {COLOR_GREEN}Answer OK:{COLOR_RESET} {final_answer}")
                else:
                    rewards["step2_answer_content"] -= 0.5
                    rollout_stats["error_type"] = "Answer Content Mismatch"
                    print(f"Rollout {rollout_idx+1}/{num_rollouts} | Step 2 | {COLOR_YELLOW}Answer Content Mismatch:{COLOR_RESET} Got '{final_answer}', Expected '{actual_tool_result}'")
            else:
    ...
    with file_path.open(mode="r", encoding="utf-8") as f:
        for line in f:
            try:
                 ...
                 print(f"{COLOR_YELLOW}Warning: Skipping invalid JSON line in {file_name}: {e}{COLOR_RESET}")
                 continue
...
def parse_args():
    ...
    parser.add_argument('--model-name', type=str, default="unsloth/Llama-3.2-1B-Instruct", help='Model name or path')
